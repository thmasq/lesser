use crossterm::event::{self, DisableMouseCapture, EnableMouseCapture, Event, KeyCode, MouseEvent, MouseEventKind};
	...
	loop {
		if last_offset != Some(offset) {
			...
			for (i, line) in display_lines.iter().take(height as usize).enumerate() {
				execute!(stdout, cursor::MoveTo(0, u16::try_from(i).unwrap_or_default()))?;
